...
**Problem:** Given CPU tasks and a cooldown `n` between repeats of the same task type, find the minimum total time (including idle slots) to complete them all. *(Also solved with a max-heap simulation in [Heap & Priority Queue](/leetcode/heap-priority-queue) — this is the greedy formula version of the same problem.)*
