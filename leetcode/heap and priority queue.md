...
**Approach:** Count frequencies, then keep a min-heap of size `k` keyed by frequency — whenever it exceeds size `k`, pop the least frequent element. (This is the general-purpose heap approach; bucket sort — see [Array & Hashing](/leetcode/array-hashing) — achieves O(n) instead when frequencies are bounded by array length.)
